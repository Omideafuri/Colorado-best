    ...
    <div className="max-w-6xl mx-auto space-y-10 pb-20 pt-4">
      ...
      <div>
        <div className="flex items-center gap-2.5 mb-3">
          ...
          <span className="text-xs tracking-brand font-semibold text-[#8C775D] uppercase">گالری فیزیکی زروی</span>
        </div>
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-[#DFD7B5] pb-8">
          <div className="max-w-xl">
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-[#2A1A08] tracking-tight mb-3">گالری شمش و مسکوکات فاخر</h1>
            <p className="text-xs sm:text-sm md:text-base text-[#57442D] leading-relaxed font-light">
              محصولات فیزیکی استاندارد با بسته‌بندی امنیتی ضدجعل، هولوگرام اصالت و شناسنامه رسمی عیار.
            </p>
          </div>
          <div className="floating-card p-5 min-w-[240px]">
            <p className="text-xs tracking-brand text-[#8C775D] mb-1 font-semibold uppercase">موجودی کیف پول ریالی</p>
            <p className="text-2xl font-bold font-num text-[#3A230A]">
              {formatNumber(cashBalanceToman)} <span className="text-xs font-normal text-[#8C775D]">تومان</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        {products.map((product, idx) => {
          ...
          return (
            <div key={product.id} className="floating-card overflow-hidden group">
              <div className="grid md:grid-cols-12">
                {/* Product Visual */}
                <div className={`md:col-span-6 bg-[#FAF8EE] flex items-center justify-center min-h-[300px] md:min-h-[380px] relative overflow-hidden p-6 sm:p-8 image-hover-zoom ${idx % 2 === 1 ? 'md:order-2' : ''}`}>
                  <div className="relative w-full h-full max-w-[280px] aspect-square rounded-2xl overflow-hidden border border-[#DFD7B5] bg-white p-4 shadow-floating-sm">
                    <Image
                      src={imageSrc}
                      alt={product.nameFa}
                      fill
                  ...
                  <div className="absolute top-5 right-5">
                    <span className={`text-[11px] font-bold tracking-wide px-3 py-1 rounded-full backdrop-blur-md ${isAvailable ? 'bg-white/95 text-[#556B54] border border-[#8DA28C]/40 shadow-floating-sm' : 'bg-white/95 text-[#8C775D] border border-[#DFD7B5]'}`}>
                ...
                <div className={`md:col-span-6 p-6 sm:p-10 flex flex-col justify-between ${idx % 2 === 1 ? 'md:order-1' : ''}`}>
                  <div>
                    <div className="flex justify-between items-start mb-3">
                      <span className="text-xs tracking-brand font-bold text-[#A4530C] uppercase">
                        قطعه ۰{toPersianDigits((idx + 1).toString())}
                      </span>
                      <span className="text-xs font-mono font-bold text-[#3A230A] uppercase">
                        {toPersianDigits(weightGrams.toString())} G
                      </span>
                    </div>

                    <h3 className="text-xl sm:text-2xl font-bold text-[#2A1A08] tracking-tight mb-1">{product.nameFa}</h3>
                    <p className="text-[11px] text-[#8C775D] mb-6 tracking-wider uppercase font-mono">{product.nameEn}</p>

                    <div className="space-y-3 border-t border-[#DFD7B5] pt-5 text-xs sm:text-sm">
                      <div className="flex justify-between items-center">
                        <span className="text-[#57442D] font-light">ارزش خام طلا</span>
                        <span className="font-num text-[#2A1A08]">{formatNumber(Number(baseValueRial / BigInt(10)))} تومان</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-[#57442D] font-light">اجرت ضرب و بسته‌بندی امنیتی</span>
                        <span className="font-num text-[#2A1A08]">{formatNumber(Number((premiumRial + makingChargeRial) / BigInt(10)))} تومان</span>
                      </div>
                      <div className="flex justify-between items-center border-t border-[#DFD7B5] pt-3.5">
                        <span className="font-bold text-[#2A1A08]">مبلغ کل قابل پرداخت</span>
                        <span className="text-xl sm:text-2xl font-bold font-num text-[#3A230A] tracking-tight">{formatNumber(totalPriceToman)} تومان</span>
                      </div>
                    </div>
                  </div>
